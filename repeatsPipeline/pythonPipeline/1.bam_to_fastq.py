...
# remove files.txt:
os.remove(rawDir + '/files.txt')

# convert files:
for inFile in fls:
	# create unique IDs for each bamfile:
	uID = os.path.basename(inFile)
	# copy the bam file across if it's not in the rawDir:
	import subprocess
	if os.path.exists(inPath + uID + '.bam'):
        subprocess.Popen("qsub -q " + subType + ".q -N cp_" + uID + " -b y -wd " +
        	logDir + " -j y -R y -pe smp " + smallcores + " -V cp'" + origRawDir +
        	"/" + uID + ".bam' " + inPath)

if whatevs in something:
     pass
else:
     something else - do nothing if condition is true, something else if false
