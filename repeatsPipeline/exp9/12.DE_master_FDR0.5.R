...
################################################################################
### htseq_EdgeR_primary_HGSOC_known_driver_vs_unknown_driver ###
      ...
      if ( i!=ctlInd ) {
        ...
        if ("repeats" %in% resultTypes) {
          ...
          if (include_ctls) {
            ...
            lab <- rbind(sig_rep, ctlGenes)
            lab$genes <- rownames(lab)
            repGenes$genes <- rownames(repGenes)
            
            # combine 'threshold' and 'type' columns:            
            lab$type_thresh <- paste0(lab$type, "_", lab$threshold)
            repGenes <- rbind(repGenes, lab)
          ...
          ######
          repGenes$type_thresh[repGenes$FDR < 0.5] <- "repeat_TRUE"
          lab <- repGenes[repGenes$FDR < 0.5,]
          lab$genes <- rownames(lab)
          lab$type_thresh <- paste0(lab$type, "_", lab$threshold)
          ######
          ...
          p <- p +  xlim(c(-4, 4))
          if (length(FCthresh) == 0) {
            if (file.exists(paste0(plotDir,   "/", Type,    "_volcano_FDR_0.5_", comp, "_reps.pdf"))) {
              print(paste0(plotDir, "/",  Type,    "_volcano_FDR_0.5_", comp, "_reps.pdf"))
              p
            } else {
              print(paste0("Creating  ",plotDir, "/", Type,    "_volcano_FDR_0.5_", comp, "_reps.pdf"))
              pdf(file = paste0(plotDir, "/",   Type,  "_volcano_FDR_0.5_", comp, "_reps.pdf"))
              print(p)
              dev.off()
            }
          } else {
            if (file.exists(paste0(plotDir, "/",  Type,  "_volcano_FDR_0.5_FC", FCthresh, "_", comp, "_reps.pdf"))) {
              print(paste0(plotDir, "/",  Type,  "_volcano_FDR_0.5_FC", FCthresh, "_", comp, "_reps.pdf already exists"))
              p
            } else {
              print(paste0("Creating  ", plotDir, "/",  Type,  "_volcano_FDR_0.5_FC", FCthresh, "_", comp, "_reps.pdf"))
              pdf(file = paste0(plotDir, "/",  Type,  "_volcano_FDR_0.5_FC", FCthresh, "_", comp, "_reps.pdf"))
              print(p)
              dev.off()
            }
          } 
