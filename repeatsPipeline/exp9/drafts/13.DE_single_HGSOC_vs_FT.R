...
# remove pAF and change categorisations of samples to either FT or HGSOC:
Counts <- Counts[,-(grep("pAF", colnames(Counts)))]
colnames(Counts) <- gsub("_[a-z][a-z][A-Z][A-Z]$", "_HGSOC", colnames(Counts))
# append '2' onto duplicate patient IDs:
#colnames(Counts)[duplicated(colnames(Counts))] <- paste0(colnames(Counts)[duplicated(colnames(Counts))], "_2")
#colnames(Counts)[grep("FT", colnames(Counts), invert=T)] <- paste0(colnames(Counts)[grep("FT", colnames(Counts), invert=T)], "_HGSOC")
