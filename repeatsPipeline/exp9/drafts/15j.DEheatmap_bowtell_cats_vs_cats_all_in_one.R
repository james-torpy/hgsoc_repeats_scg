...
for (i in 1:length(spl_fc)) {
  print(i)
  pdf(file=paste0(plotDir, "/log2FC_HGSOC_heatmap", i, ".pdf"), width=10, height=10)
  pheatmap(spl_fc[[i]], color = colorRampPalette(c("#08519C", "white", "firebrick3"))(50),
           display_numbers = as.matrix(ifelse(spl_fdr[[i]] < 0.05, "*", "")), fontsize = 8,
           scale="column")
  dev.off()
}
...
interesting_fc <- fcDF %>%
  ...
  dplyr::filter(rowSums(fcDF > 2 | fcDF < -2) >= 1) %>%
  column_to_rownames('gene_id')

interesting_fdr <-  fdrDF[rownames(interesting_fc),]

pdf(file=paste0(plotDir, "/log2FC_HGSOC_heatmap_interesting_all_in_one.pdf"), width=10, height=10)
