...
### 1. Load in all counts and remove pAF, rcAF samples ###

custom3Counts <- readRDS(paste0(RobjectDir, "/", Type,
                                "_allcounts.htseq.rds"))
custom3Counts <- custom3Counts[,grep("pAF|rcAF", colnames(custom3Counts), invert = T, value = T)]


gcCounts <- readRDS(paste0(RobjectDir,
                           "/gc_allcounts.htseq.rds"))
gcCounts <- gcCounts[,grep("pAF|rcAF", colnames(gcCounts), invert = T, value = T)]

# append gcCounts to custom3Counts:
Counts <- rbind(custom3Counts, gcCounts)

# change the subtype labels to clearer names:
HGSOCtypes <- c("acquired_resistant", "extreme_responders", "FT", "multiple_responders", "metastatic", "primary_resistant", "refractory")

colnames(Counts) <- gsub(
  "arPT", "acquired_resistant", gsub(
    "erPT", "extreme_responders", gsub(
      "mrPT", "multiple_responders", gsub(
        "msST", "metastatic", gsub(
          "prPT", "primary_resistant", gsub(
            "rfPT", "refractory", colnames(Counts)
          )
        )
      )
    )
...
sTypes <- unique(
  grep(
    "id", gsub(
      "^.*_[0-9][0-9][0-9]_", "", colnames(Counts)
    ), value=T, invert = T
  )
)
...
colOrder <- unlist(split(colnames(Counts), gsub("^.*_[0-9][0-9][0-9]_", "", colnames(Counts))))
Counts <- Counts[,colOrder]

splt <- unlist(lapply(split(colnames(Counts), gsub("^.*_[0-9][0-9][0-9]_", "", colnames(Counts))), length))
...
for (i in 1:ncol(design)) {
  ...
  if (i!=ctlInd) {
    ...
    if (file.exists(paste0(newRobjectDir, sTypes[i],   "_vs_FT_lrt.rds"))) {
      print(paste0(newRobjectDir, sTypes[i],   "_vs_FT_lrt.rds already exists, no need to create"))
    } else {
      print(paste0("Creating ", newRobjectDir, sTypes[i],   "_vs_FT_lrt.rds"))
      saveRDS(lrt, file = paste0(newRobjectDir, sTypes[i],   "_vs_FT_lrt.rds"))
    }
    ...
    rownames(posGenes) <- c("CD47", "GAPDH")
...
allReps <- c(allReps[1:2], allReps[4:length(allReps)])
names(allReps) <- paste0(sTypes[-ctlInd], "_vs_FT")

sigReps <- c(sigReps[1:2], sigReps[4:length(sigReps)])
names(sigReps) <- paste0(sTypes[-ctlInd], "_vs_FT")

epiGenes <- append(epiGenes[1:2], epiGenes[4:length(epiGenes)])
names(epiGenes) <- paste0(sTypes[-ctlInd], "_vs_FT"])
