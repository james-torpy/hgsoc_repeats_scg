...
for (i in 1:length(allGene)) {
  ...
  pdf(file=paste0(plotDir, "/log2FC_HGSOC_heatmap_", names(allGene)[i], ".pdf"), width=10, height=10)
  pheatmap(fcDF, color = colorRampPalette(c("#08519C", "white", "firebrick3"))(50),
           display_numbers = as.matrix(ifelse(fdrDF < 0.1, "*", "")), fontsize = 8,
           scale="column")
  dev.off()
  ...
  pdf(file=paste0(plotDir, "/log10_FDR_HGSOC_heatmap_", names(allGene)[i], ".pdf"), width=10, height=10)
  ...
  pdf(file=paste0(plotDir, "/log2FC_HGSOC_heatmap_interesting_", names(allGene)[i], ".pdf"), width=10, height=10)
