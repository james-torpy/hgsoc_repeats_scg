...
if slots < lower_core_lim:
    ...
    in_files = glob.glob(raw_dir + '/*.bam.gz')
            ...
            if u_id not in done_ids:
                ...
                for i in range(total_no_jobs):
                    ...
                    if not all([os.path.isfile(inp) for inp in inputs]):
                        print('At least one job f' + ' input file does not exist, holding job')
        
                    elif all([os.path.isfile(inp) for inp in inputs]) and not all([os.path.isfile(outp) \
                    	for outp in outputs]) and not 'jf' + '_' + u_id in qstat_jobs:
                        print('Submitting job f' + ' with input/s or in dir/s:')
                        print(inputs)
                        print('')
                    ...
                    elif 'jf' + '_' + u_id in redo and not 'jf' + '_' + u_id in qstat_jobs:
                        cores[i] = int(cores[i]) + 2
                        print('Job previously encountered an error!')
                        print('Submitting job f' + ' with input/s or in dir/s and ' + str(cores[i]) + ' cores:')
                        print(inputs)
                        print('')
                        print('and output/s or out dir/s:')
                        print(outputs)
                    ...
                    # run job if outputs exist but are not the minimum file size:
                        ...
                        print('Job f' + ' has previously been completed, no need to run')
                    
                    elif 'jf' + '_' + u_id in qstat_jobs:
                        os.system('echo Job f' + ' is currently running or queued, no need to run again')
