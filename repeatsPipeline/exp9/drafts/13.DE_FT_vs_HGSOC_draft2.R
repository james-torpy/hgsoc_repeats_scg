...
#fit <- readRDS(file=paste0(newRobjectDir, "/", Type, "DEfit.rds"))
...
for (i in 1:length(STypes)) {
  if (i!=1) {
    ...
    create_rds(lrt, paste0(newRobjectDir, "FT_vs_", STypes[i],   "_lrt.rds"))
...
#load(file = paste0(newRobjectDir, "DEImg_", expName, ".RData"))
