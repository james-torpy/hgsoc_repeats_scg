...
FTind <- grep("FT", colnames(design))
con <- c(rep(0, (FTind - 1) ), -1, rep(0, (ncol(design) - FTind)))
...
allReps <- append(allReps[1:2], allReps[4:5])
names(allReps) <- c(paste0("FT_vs_", sTypes[1:2]), paste0("FT_vs_", sTypes[4:5]))

sigReps <- append(sigReps[1:2], sigReps[4:5])
names(sigReps) <- c(paste0("FT_vs_", sTypes[1:2]), paste0("FT_vs_", sTypes[4:5]))

epiGenes <- append(epiGenes[1:2], epiGenes[4:5])
names(epiGenes) <- c(paste0("FT_vs_", sTypes[1:2]), paste0("FT_vs_", sTypes[4:5]))
