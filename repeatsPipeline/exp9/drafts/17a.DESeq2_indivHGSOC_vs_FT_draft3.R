...
# Run on cluster with:
#briR
#qsub -N DESeq2 -b y -wd \
#/share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9/DE \
#-j y -R y -pe smp 12 -V "Rscript /share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/repeatsPipeline/exp9/17a.DESeq2_indivHGSOC_vs_FT_draft3.R"
...
if (file.exists(paste0(RobjectDir, "/post_DESeq.RData"))) {
	...
	nSet <- betweenLaneNormalization(set,
	...
	### 4. Perform DESeq on counts ###
	
	# define condition groups:
	condition <- factor(gsub("^.*FT", "FT",colnames(Counts)), levels = c("FT", colnames(Counts)[grep("FT", colnames(Counts), invert=T)]))
	...
	dds <- DESeqDataSetFromMatrix(countData = assayData(nSet)$counts,
	                              colData = coldata,
	                              design= ~ condition)
	
	EDASeqNormFactors <- exp(-1 * assayData(nSet)$offset)
	
	normalizationFactors(dds) <- EDASeqNormFactors
	
	dds <- estimateDispersions(dds)
	save.image(file=paste0(RobjectDir, "/post_dispEst.RData"))
	
	dds <- nbinomWaldTest(dds)
	save.image(file=paste0(RobjectDir, "/post_DESeq.RData"))
	
	pdf(paste0(plotDir, "/postDESeq2_RLE.pdf"))
	boxplot(log10(assays(dds)[["cooks"]]), range=0, las=2, cex.axis=0.3)
	dev.off()
}


for (k in seq(5, length(resultsNames(dds)), 5)) {
	#register(MulticoreParam(workers=8))
	system.time(for (i in (k-4):k) {
	  print(i)
	  if (i==2) {
#	    resLFC <- list(lfcShrink(dds, coef=i, parallel=TRUE))
		resLFC <- list(lfcShrink(dds, coef=i))
	  } else if (i>2) {
	  	#resLFC[[i]] <- lfcShrink(dds, coef=i, parallel=TRUE)
	    resLFC[[i]] <- lfcShrink(dds, coef=i)
	  }
	})
	print("Next batch of 5 beginning")
}


names(resLFC) <- c(resultsNames(ddsDE)[1:11], resultsNames(ddsDE)[1:length(resultsNames(ddsDE))])

save.image(file=paste0(RobjectDir, "/post_lfcShrink.RData"))
