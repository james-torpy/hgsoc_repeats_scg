...
if ( EdgeR == TRUE) {
  ...
  temp2$type <- "sig"
  temp2$threshold <- FALSE
  temp2$threshold[temp2$adjPvalue_FDR < 10e-6] <- TRUE
  temp2$typethresh <- paste0(temp2$type, temp2$threshold)
...
# create smoothScatter plot with regression line:
reg <- lm(my_logFC~Patch_logFC, data = FC)

pdf(file = paste0(plotDir, "/htseq_vs_patch_CCNEamp_vs_HRD_smoothscatter.pdf"))
smoothScatter(x=FC$my_logFC, y=FC$Patch_logFC, xlim=c(-4, 4), ylim=c(-4, 4))
abline(reg, col="red")
dev.off()
...
save.image(paste0(RobjectDir, "/19.patch_compare_final.Rdata"))
