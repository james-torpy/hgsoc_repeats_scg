...
for ( i in 1:nrow(sup_CPM) ) {
  for ( j in 1:nrow(L1_CPM) ) {
    print(paste0("Finding correlation for ", rownames(sup_CPM[i,]),
      " and ", rownames(L1_CPM[j,])))
    
    # create data frame with suppressor CPMs and mean L1 CPMs:
    df <- data.frame(as.numeric(sup_CPM[i,]), as.numeric(L1_CPM[j,]))
    colnames(df) <- c("suppressor_CPM", "L1_CPM")
    ...
    L1_ID <- rownames(L1_CPM[j,])
    
    # if correlation < -0.5, save as scatterplot:
    if ( sup_cor < -0.5 ) {
      log_df <- log10(df)
      
      reg <- lm(L1_CPM~suppressor_CPM, data = log_df)
      
      # reg <- lm(suppressor_CPM~mean_L1_CPM, data = log_df)
      xmax <- max(log_df$suppressor_CPM)
      ymax <- max(log_df$L1_CPM)
      
      pdf(paste0(plotDir, "/", sym, "_suppressor_correlation_with_", 
                 rownames(L1_CPM[j,]), "_exp.pdf"))
      par(mar = c(5,5,3,3))
      plot(log_df, ann = F)
      abline(reg, col="red")
      ...
      if ( !(exists("correlated_sups_0.5")) ) {
        correlated_sups_0.5 <- c(paste0(L1_ID, "_vs_", sym))
      } else {
        correlated_sups_0.5 <- c(correlated_sups_0.5, paste0(L1_ID, "_vs_", sym))
      }
    }
    ...
    if ( sup_cor < -0.3 ) {
      if ( !(exists("correlated_sups_0.3")) ) {
        correlated_sups_0.3 <- c(paste0(L1_ID, "_vs_", sym))
      } else {
        correlated_sups_0.3 <- c(correlated_sups_0.3, paste0(L1_ID, "_vs_", sym))
      }
    }
  }
}
write.table(correlated_sups_0.3, paste0(tableDir, 
  "/L1_suppressors_correlated_with_L1_CPMs_0.3.txt"), quote = F, sep = "\n")
write.table(correlated_sups_0.5, paste0(tableDir, 
  "/L1_suppressors_correlated_with_L1_CPMs_0.5.txt"), quote = F, sep = "\n")
...
for ( i in 1:nrow(act_CPM) ) {
  for ( j in 1:nrow(L1_CPM) ) {
    ...
    # create data frame with act CPMs and mean L1 CPMs:
    df <- data.frame(as.numeric(act_CPM[i,]), as.numeric(L1_CPM[j,]))
    colnames(df) <- c("act_CPM", "L1_CPM")
    df <- df[apply(df, 1, function(row) all(row !=0 )),]
    ...
    if ( act_cor > 0.5 ) {
      ...
      reg <- lm(L1_CPM~act_CPM, data = log_df)
      
      # reg <- lm(act_CPM~mean_L1_CPM, data = log_df)
      xmax <- max(log_df$act_CPM)
      ymax <- max(log_df$L1_CPM)
      ...
      if ( act_cor > 0.5 ) {
        if ( !(exists("correlated_acts_0.5")) ) {
          correlated_acts_0.5 <- c(paste0(L1_ID, "_vs_", sym))
        } else {
          correlated_acts_0.5 <- c(correlated_acts_0.5, paste0(L1_ID, "_vs_", sym))
        }
      }
    }
    ...
    if ( act_cor > 0.3 ) {
      if ( !(exists("correlated_acts_0.3")) ) {
        correlated_acts_0.3 <- c(paste0(L1_ID, "_vs_", sym))
      } else {
        correlated_acts_0.3 <- c(correlated_acts_0.3, paste0(L1_ID, "_vs_", sym))
      }
    }
  }
}
write.table(correlated_acts_0.3, paste0(tableDir, 
  "/L1_acts_correlated_with_L1_CPMs_0.3.txt"), quote = F, sep = "\n")
write.table(correlated_acts_0.5, paste0(tableDir, 
  "/L1_acts_correlated_with_L1_CPMs_0.5.txt"), quote = F, sep = "\n")
...
for ( i in 1:nrow(ctl_CPM) ) {
  for ( j in 1:nrow(L1_CPM) ) {
    ...
    df <- data.frame(as.numeric(ctl_CPM[i,]), as.numeric(L1_CPM[j,]))
...
# save list of correlated L1 regulators:
cor_s <- data.frame(unique(gsub("^.*_", "", correlated_sups_0.3)), "suppressor")
colnames(cor_s) <- c("symbol", "effect")
cor_a <- data.frame(unique(gsub("^.*_", "", correlated_acts_0.3)), "activator")
colnames(cor_a) <- c("symbol", "effect")
cor_regs <- rbind(cor_s, cor_a)

# add ensembl_ids:
m <- match(cor_regs$symbol, egSYMBOL$symbol)
gene_id <- egSYMBOL$gene_id[m]
m <- match(gene_id, egENSEMBL$gene_id)
cor_regs$ensembl_id <- egENSEMBL$ensembl_id[m]

write.table(cor_regs, paste0(refDir, "/correlated_liu_2018_L1_regulators.txt"), 
