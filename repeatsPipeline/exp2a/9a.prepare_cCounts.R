...
sampleNames <- unique(gsub(paste0("_", Type, "RepeatCounts_half[1-2]a.rds"), "", basename(countFiles)))

j=1
for (i in seq(1, length(countFiles), 2)) {
  # define sampleName:
  sampleName <- gsub(paste0("_", Type, "RepeatCounts_half[1-2]a.rds"), "", basename(countFiles[i]))
  # append half 1 of sample counts to half 2 and add to list:
  if (i==1) {
    Counts <- list(c(readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half1a.rds")), readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half2a.rds"))))
  } else {
    Counts[[j]] <- c(readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half1a.rds")), readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half2a.rds")))
  }  
  j=j+1
}
