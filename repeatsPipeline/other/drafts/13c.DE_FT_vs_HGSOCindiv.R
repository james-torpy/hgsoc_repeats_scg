...
RobjectDir_load <- paste0(projectDir, "/RNA-seq/Robjects/",
                          expName, "/")
RobjectDir <- paste0(projectDir, "/RNA-seq/Robjects/",
  expName, "/test/individual_patients/")
plotDir <- paste0(resultsDir, "/R/", expName,
  "/plots/DEplots/test/individual_patients/")
...
custom3Counts <- readRDS(paste0(RobjectDir_load, "/", Type,
  "_allcounts.htseq.rds"))
gcCounts <- readRDS(paste0(RobjectDir_load,
...
# make factor vector identifying FT sample types and leaving other sample types as their original names:
no <- length(grep("FT", colnames(Counts)))
typeF <- c(rep("FT", no), colnames(Counts)[(no+1):ncol(Counts)])
levels(typeF) <- c("FT", colnames(Counts)[(no+1):ncol(Counts)])
...
con <- c(rep(0, (ncol(design)-1)), -1)
for (i in 1:ncol(design)) {
  if (!colnames(design)[i] =="typeFFT") {
    comp <- paste0("FT_vs_", gsub("typeF", "", colnames(design)[i]))
...
names(allReps) <- paste0("FT_vs_", colnames(design)[-ncol(design)])
names(sigReps) <- paste0("FT_vs_", colnames(design)[-ncol(design)])
