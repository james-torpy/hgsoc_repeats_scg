...
# eliminate lowly expressed genes (rows where there are less than 3
# counts where df > 4):
...
#colnames(Counts)[duplicated(colnames(Counts))] <- paste0(colnames(Counts)
# [duplicated(colnames(Counts))], ".2")
...
typeF <- factor(gsub("^.*FT", "FT", colnames(Counts)), levels = c("FT", 
  colnames(Counts)[!(colnames(Counts) %in% "FT")]))
...
colnames(Counts)[duplicated(colnames(Counts))] <- gsub("_HGSOC", ".2_HGSOC", 
...
if (file.exists(paste0(plotDir, "/", Type, "_RLElaneNormGC.pdf"))) {
  print(paste0(plotDir, "/", Type, "_RLElaneNormGC.pdf already exists, 
    no need to create"))
} else {
...
if (file.exists(paste0(plotDir, "/", Type, "_pcalaneNormGC.pdf"))) {
  print(paste0(plotDir, "/", Type, "_pcalaneNormGC.pdf already exists, 
    no need to create"))
} else {
  print(paste0("Creating ", plotDir, "/", Type, "_pcalaneNormGC.pdf"))
  pdf(file = paste0(plotDir, "/", Type, "_pcalaneNormGC.pdf"), height = 15, 
    width = 20)
...
### 4. Perform differential expression comparing normalised FT controls to 
# cancer samples ###

# design matrix labelling all sample types:
design <- model.matrix(~0+typeF, data=pData(nSet))

# convert set into a DGElist format which specifies library size and normalisation 
# factors:
y <- DGEList(counts=assayData(nSet)$normalizedCounts, group=typeF)
...
# calculate the deviance residuals from a first-pass GLM regression of the counts on 
# the co-variates of interest (p8 RUVseq manual):
# estimate dispersion:
  ...
  if (i!=ctlInd) {
    ...
    if ("repeats" %in% resultTypes) {
      ...
      if (length(FCthresh) == 0) {
        ...
        sigGenes <- filter(
          repGenes, (FDR < FDRthresh & logFC < -(FCthresh))|(FDR < FDRthresh & 
            logFC > FCthresh)
        )
        repGenes$threshold <- as.factor(
          (repGenes$FDR < FDRthresh & repGenes$logFC < -(FCthresh))|(repGenes$FDR 
            <  FDRthresh & repGenes$logFC > FCthresh)
        )
      }
      ...
      p <- p +  xlim(c(-4, 4))
      if (length(FCthresh) == 0) {
        if (file.exists(paste0(plotDir,   "/", Type,  "_volcano_FDR_",   FDRthresh, 
          "_", comp, ".pdf"))) {
          print(paste0(plotDir, "/",  Type,  "_volcano_FDR_",   FDRthresh, "_", comp, 
            ".pdf"))
          p
        } else {
          print(paste0("Creating  ",plotDir, "/", Type,    "_volcano_FDR_", FDRthresh, 
            "_", comp, ".pdf"))
          pdf(file = paste0(plotDir, "/",   Type,  "_volcano_FDR_",  FDRthresh, "_", 
            comp, ".pdf"))
          print(p)
          dev.off()
        }
      } else {
        if (file.exists(paste0(plotDir, "/",  Type,  "_volcano_FDR",   FDRthresh, "_FC", 
          FCthresh, "_", comp, ".pdf"))) {
          print(paste0(plotDir, "/",  Type,  "_volcano_FDR",   FDRthresh, "_FC", FCthresh, 
            "_", comp, ".pdf already exists"))
          p
        } else {
          print(paste0("Creating  ", plotDir, "/",  Type,  "_volcano_FDR", FDRthresh, "_FC", 
            FCthresh, "_", comp, ".pdf"))
          pdf(file = paste0(plotDir, "/",  Type,  "_volcano_FDR",   FDRthresh, "_FC", FCthresh, 
            "_", comp, ".pdf"))
          print(p)
          dev.off()
        }
      ...
      if ("epiMods" %in% resultTypes) {
        ...
        if (length(FCthresh) == 0) {
          ...
          epiGenes$threshold <- as.factor((epiGenes$FDR < FDRthresh & 
            epiGenes$logFC < -(FCthresh))|(epiGenes$FDR < FDRthresh & epiGenes$logFC > FCthresh))
        }
        ...
        p <- p +  xlim(c(-2, 2))
        if (length(FCthresh) == 0) {
          if (file.exists(paste0(plotDir,   "/", Type,  "_volcano_FDR_",   FDRthresh, "_", 
            comp, "_epi.pdf"))) {
            print(paste0(plotDir, "/",  Type,  "_volcano_FDR_",   FDRthresh, "_", comp, 
              "_epi.pdf"))
            p
          ...
          if (file.exists(paste0(plotDir, "/",  Type,  "_volcano_FDR",   FDRthresh, "_FC", 
            FCthresh, "_", comp, "_epi.pdf"))) {
            print(paste0(plotDir, "/",  Type,  "_volcano_FDR",   FDRthresh, "_FC", FCthresh, 
              "_", comp, "_epi.pdf already exists"))
            p
          } else {
            print(paste0("Creating  ", plotDir, "/",  Type,  "_volcano_FDR", FDRthresh, "_FC", 
              FCthresh, "_", comp, "_epi.pdf"))
...
### 8. Save all repeat DE, and all repeat DE that is significant and log2 FC > 1 for at least one 
# comparison:
