### 13d.DE_ccne_vs_hrd_both_unknown.R ###
...
plotDir <- paste0(resultsDir, "/R/", expName,
                  "/plots/DEplots/ccne_vs_hrd_unknown/")
...
### 4. Perform differential expression comparing normalised CCNE controls to cancer samples ###
...
allReps <- append(allReps[1], allReps[3:4])
names(allReps) <- c(paste0("CCNE_vs_", sTypes[1]), paste0("CCNE_vs_", sTypes[3:4]))

sigReps <- append(sigReps[1], sigReps[3:4])
names(sigReps) <- c(paste0("CCNE_vs_", sTypes[1]), paste0("CCNE_vs_", sTypes[3:4]))
...
# fetch allSig for hrd_vs_ccne_unknown and append onto totalSig to include in new allSig:
hrdAllSig <- readRDS(paste0(projectDir, "/RNA-seq/Robjects/", expName, "/hrd_vs_ccne_unknown/", Type, "_DEsigReps.rds"))
totalSig <- unique(append(totalSig, rownames(hrdAllSig[[1]])))

# fetch all the entries for the above significant repeat names and save:
allSig <- lapply(allReps, function(x) {
  return(x[totalSig,])
})
#allSig <- append(allSig[1], allSig[3:4])
#names(allSig) <- c(paste0("CCNE_vs_", sTypes[1]), paste0("CCNE_vs_", sTypes[3:4]))

saveRDS(allSig, file=paste0(newRobjectDir, "/", Type, "_DEsigReps_with_hrd_vs_ccne_unknown.rds"))
