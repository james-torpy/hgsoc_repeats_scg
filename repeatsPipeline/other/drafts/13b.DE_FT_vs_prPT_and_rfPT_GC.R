...
plotDir <- paste0(resultsDir, "/R/", expName,
  "/plots/DEplots/FT_vs_prPT_and_rfPT/test/")
...
for (i in 1:length(STypes)) {
  if (i!=1) {
    ...
    allGenes$threshold <- "NON-SIG"
    if (nrow(allGenes[allGenes$FDR<thresh,])>0) {
      allGenes[allGenes$FDR<thresh,]$threshold <- "SIG"
    }
    ...
    p <- p + geom_text_repel(data=allGenes, aes(label=genes))
...
if (file.exists(paste0(plotDir, "/", Type,  "_volcano_FDR", thresh, comp, ".pdf"))) {
    ...
    intrGenes$threshold = "INTEREST"
    if (nrow(intrGenes[intrGenes$FDR<thresh,])>0) {
      intrGenes[intrGenes$FDR<thresh,]$threshold <- "INTRSIG"
    }
    
    bothGenes <- rbind(repGenes, intrGenes)
    
    lab <- intrGenes
    lab$genes <- rownames(lab)
    
    
    p <- ggplot(data=bothGenes, aes(x=logFC, y=-log10(FDR),  color=threshold))
    ...
    p <- p +  xlim(c(-4, 4))
    p <- p + ylim(c(0, 4))
    p <- p + scale_colour_manual(values = c("forestgreen", "deeppink1", "grey"))
    if (file.exists(paste0(plotDir, "/", Type,  "_volcano_FDR", thresh, "_logFC", FCthresh, "_", comp,  "_intgenes.pdf"))) {
      print(paste0(plotDir, "/", Type,  "_volcano_FDR", thresh, "_logFC", FCthresh, "_", comp, "_intgenes.pdf"))
      p
    } else {
      print(paste0("Creating ",plotDir, "/", Type,  "_volcano_FDR", thresh, "_logFC", FCthresh, "_", comp,  "_intgenes.pdf"))
      pdf(file = paste0(plotDir, "/", Type,  "_volcano_FDR", thresh, "_logFC", FCthresh, "_", comp, "_intgenes.pdf"))
      p
      dev.off()
    }
    ...
    #allGenes$threshold <- as.factor((allGenes$FDR < thresh & allGenes$logFC < -(FCthresh))|(allGenes$FDR <  thresh & allGenes$logFC > FCthresh))
