...
plotDir <- paste0(resultsDir, "/R/", expName,
  "/plots/DEplots/FT_vs_erPT_and_mrPT/")
...
# rename erPT, mrPT as rPT and select only FT and rPT:
colnames(Counts) <- gsub("erPT|mrPT", "rPT", colnames(Counts))
Counts <- Counts[,grep("FT|\\_rPT", colnames(Counts))]
