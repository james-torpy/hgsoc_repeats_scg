...
RobjectDir <- paste0(projectDir, "/RNA-seq/Robjects/",
  expName, "/hrd_ccne_unknown/")
plotDir <- paste0(resultsDir, "/R/", expName,
  "/plots/DEplots/hrd_ccne_unknown/")
...
# split FT counts from others:
FTcounts <- Counts[,grep("_FT", colnames(Counts))]
otherCounts <- Counts[,grep("_FT", colnames(Counts), invert=T)]

# remove existing categories from otherCounts:
colnames(otherCounts) <- gsub("_[a-z].*$", "", colnames(otherCounts))
Counts <- cbind(FTcounts, otherCounts)
