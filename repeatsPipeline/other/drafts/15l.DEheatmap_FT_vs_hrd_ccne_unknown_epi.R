...
pdf(file=paste0(plotDir, "/log2FC_HGSOCvsFT_heatmap_epi.pdf"), width=10, height=10)
pheatmap(fcDF, color = colorRampPalette(c("#08519C", "white", "firebrick3"))(50),
         display_numbers = as.matrix(ifelse(fdrDF < 0.1, "*", "")), fontsize = 8,
         scale="column")
dev.off()
...
pdf(file=paste0(plotDir, "/log10_FDR_HGSOCvsFT_heatmap_epi.pdf"), width=10, height=10)
pheatmap(logfdrDF, color = colorRampPalette(c("#08519C", "white"))(50), display_numbers = as.matrix(ifelse(fdrDF < 0.1, "*", "")), fontsize = 8)
dev.off()
 
interesting_fc <- fcDF %>%
  rownames_to_column('gene_id') %>%
  dplyr::filter(rowSums((fdrDF) < 0.1 & (fcDF > 1 | fcDF < -1)) >= 1) %>%
  column_to_rownames('gene_id')

interesting_fdr <-  fdrDF[rownames(interesting_fc),]

if (nrow(interesting_fc) > 1) {
  pdf(file=paste0(plotDir, "/log2FC_HGSOCvsFT_heatmap_interesting_epi_.pdf"), width=10, height=10)
