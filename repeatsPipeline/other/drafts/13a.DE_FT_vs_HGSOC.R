...
for (i in 1:ncol(design)) {
  if (i!=ctlInd) {
    ...
    if (file.exists(paste0(RobjectDir, "FT_vs_", sTypes[i],   "_lrt.rds"))) {
      print(paste0(RobjectDir, "FT_vs_", sTypes[i],   "_lrt.rds already exists, no need to create"))
    } else {
      print(paste0("Creating ", RobjectDir, "FT_vs_", sTypes[i],   "_lrt.rds"))
      saveRDS(lrt, file = paste0(RobjectDir, "FT_vs_", sTypes[i],   "_lrt.rds"))
    }
...
saveRDS(epiGenes, file=paste0(RobjectDir, "/", Type, "_DEepiGenes.rds"))
