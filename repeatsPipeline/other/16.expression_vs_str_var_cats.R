  ...
  for (k in 1:length(rpInt)) {
    if ( length(rpInt) > 0 ) {
      if ( !is.null(rpInt[[k]]) ) {
        ...
        if ( !file.exists(paste0(plotDir, "/", rpName, "_DE_", names(orders)[m], "_", corr_thresh, 
                                 "_corr_thresh.pdf")) ) {
          pdf(file = paste0(plotDir, "/", rpName, "_DE_", names(orders)[m], "_", corr_thresh, 
                            "_corr_thresh.pdf"), width=10, height=10)
  ...
  for (k in 1:length(rpInt)) {
    if ( length(rpInt) > 0 ) {
      if ( !is.null(rpInt[[k]]) ) {
        print(k)
        rpName <- gsub("\\(|\\)", "", names(rpInt)[k])
        if ( (cors[rpName] > 2 | cors[rpName] < -2) ) {
          if ( !file.exists(paste0(plotDir, "/", rpName, "_CPMR_", names(orders)[m], "_", corr_thresh,
                                   "_corr_thresh.pdf")) ) {
            pdf(file = paste0(plotDir, "/", rpName, "_CPMR_", names(orders)[m], "_", corr_thresh,
                              "_corr_thresh.pdf"), width=10, height=10)
