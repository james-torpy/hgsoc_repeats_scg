...
Counts <- readRDS(file=paste0(RobjectDir, annot, "_RepeatCounts/all_", annot, "RepeatCountDFs_b.rds"))
if (annot=="custom3") {
  Counts2 <- readRDS(file=paste0(RobjectDir, "c_RepeatCounts/all_cRepeatCountDFs_b.rds"))
}
...
gcFiles <- list.files(paste0(RobjectDir, "/GCcountsDF"), pattern="GCcountsDFb", full.names=T)
gcFiles <- grep("Ext", grep("subset", gcFiles, value=T, invert=T), value=T, invert=T)
gcFiles <- grep("rGC", gcFiles, value=T)
gcL <- list()
j=1
for (f in gcFiles) {
  gcL[[j]] <- as.data.frame(readRDS(file = f)[,1])
  GCrownames <- rownames(readRDS(file = f))
  j=j+1
}
...
# organise non-reduced GCcounts to
...
create_rds(lrt, paste0(RobjectDir, "FT_vs_prPT", "_lrt.rds"))
...
repGenes <- allGenes[grep("^[0-9]*.$", rownames(allGenes), invert = T),]
...
ctlGenes <- readRDS(paste0(projectDir, "/RNA-seq/Robjects/exp2/", annot, "_allGenesDEa.rds"))
posGenes <- rbind(ctlGenes["ENSG00000091831",], ctlGenes["ENSG00000196776",], ctlGenes["ENSG00000105173",])
negGenes <- rbind(ctlGenes["ENSG00000111640",], ctlGenes["ENSG00000075624",], ctlGenes["ENSG00000204574",],
                  ctlGenes["ENSG00000104904",], ctlGenes["ENSG00000089157",])
