...
if (file.exists(paste0(RobjectDir, "/", annot, "_DEreps.rds"))) {
  ...
  for (i in 1:length(CTypes)) {
    if (i!=3) {
      ...
      # plot on volcano plot FDR<0.05, FC>=2:
      ...
      p <- p + labs(x="log2 fold change vs FT control", y="-log10 FDR")
      p <- p +  xlim(c(-7, 7))
      if (file.exists(paste0(plotDir, "/", annot, "_volcano_FDR0.05_FC3_", comp, ".pdf"))) {
        print(paste0(plotDir, "/", annot, "_volcano_FDR0.05_FC3_", comp, ".pdf"))
        p
      } else {
        print(paste0("Creating ", plotDir, "/", annot, "_volcano_FDR0.05_FC3_", comp, ".pdf"))
        pdf(file = paste0(plotDir, "/alt/", annot, "_volcano_FDR0.05_FC3_", comp, ".pdf")) 
        print(p)
        dev.off()
      }
      ...
      if (file.exists(paste0(plotDir, "/", annot, "_volcano_FDR0.05_FC2_", comp, ".pdf"))) {
        ...
        pdf(file = paste0(plotDir, "/alt/", annot, "_volcano_FDR0.05_FC2_", comp, ".pdf")) 
        print(p)
        dev.off()
      }
