...
gcFiles <- grep(
  "ext", list.files(paste0(RobjectDir, "/gcCounts"), full.names = T), value = T, invert = T
  )

Counts <- subset(cbind(readRDS(file=gcFiles[68]), readRDS(file=gcFiles[1])), select=- )
colnames(Counts) <- c("gene_id", "AOCS91_acquired_resistance", "AOCS91_recurrent_ascites")
