...
nCounts <- normalize.quantile.robust(dCounts)
colnames(nCounts) <- colnames(Counts)
rownames(nCounts) <- rownames(Counts)
pdf(file = paste0(plotDir, "htseq_", Type, "_RLElaneNormGC.pdf"))
plotRLE(dCounts, outline=FALSE, ylim=c(-4, 4))
dev.off()
...
Counts <- Counts[,!colnames(Counts) %in% c("AOCS_175_FT", "AOCS_083_prPT", "AOCS_085_prPT", "AOCS_107_prPT", "AOCS_111_prPT", "AOCS_113_prPT", "AOCS_116_prPT", "AOCS_163_prPT")]
dCounts <- Counts
storage.mode(dCounts) <- "double"
...
saveRDS(sig, file=paste0(RobjectDir, "/", Type, "_DEreps.rds"))
