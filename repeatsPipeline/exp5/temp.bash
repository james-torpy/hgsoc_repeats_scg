...
#	qsub -q $subType.q -N sc2_$uID -hold_jid gz1_$uID,gz2_$uID,gz3_$uID -b y -wd $logDir -j y -R y \
#	-pe smp $smallcores \
#	$scriptsPath/2.fastq_report_gen.bash $uID $outPath $projectDir \
#	$inFile
#	# 3a.starRibo.bash:
#	qsub -q $subType.q -N sc3a_$uID -hold_jid sc2_$uID -b y -wd $logDir -j y -R y \
#	-pe smp $largecores \
...
#	$largecores
	...
	-pe smp $largecores \
	$scriptsPath/$expName/3d.starGCchim.bash
	
	# 4.bam_report_gen.bash:
	qsub -q $subType.q -N sc4_$uID -hold_jid sc3a_$uID,sc3b_$uID -b y -wd $logDir -j y -R y \
	-pe smp $smallcores \
	$scriptsPath/4.bam_report_gen.bash $uID $outPath
	
	# 5a.htseq.bash:
	export uID=$uID
	export numcores=$largecores
...
#	-V $scriptsPath/$expName/5b.htseq-rep_c.bash
	...
	# 6.compGen.R:
	qsub -q $subType.q -N sc6_$uID -hold_jid j5a_$uID,j5b_$uID,j5c_$uID -b y -wd $logDir -j y -R y -P \
	-pe smp $countcores -V 6.compGen.R
