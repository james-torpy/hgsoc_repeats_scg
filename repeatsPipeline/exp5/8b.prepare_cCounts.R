...
countFiles <- grep(
	paste0("subset|_custom|all"), grep(
		paste0(Type, "_"), 
...
c_countFiles <- grep("gc_", countFiles, value=T, invert=T)
gc_countFiles <- grep("gc_", countFiles, value=T)

######

c_countFiles <- c_countFiles[2:7]

######

sampleNames <- gsub("\\.htseq.txt", "", gsub(paste0(Type, "_"), "", basename(c_countFiles)))
