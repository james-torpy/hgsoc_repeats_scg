...
for (Type in list("c", "custom3")) {
    ...
    # dataframe:
    countDFlist <- lapply(countFiles, function(x) {
      for (i in 1:length(x)) {
        if (i==1) {
          tab <- read.table(x[i])
    ...
    Counts <- as.data.frame(do.call("cbind", countDFlist))
    ...
    newNames <- paste0(Key$V4, "_", gsub("[0-9]", "", colnames(Counts)))
    colnames(Counts) <- newNames
