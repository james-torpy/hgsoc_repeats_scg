...
htseq_line="htseq-count -f bam -i ID -t exon --stranded=no -o $outDir/$uID.$Type.out.sam $inFile $gff >> $outDir/$uID.$Type.htseq.txt"
echo This is the htseq_line:
echo $htseq_line
echo -e

htseq-count -f bam -i ID -t exon --stranded=no -o $outDir/$uID.$Type.out.sam $inFile $gff >> $outDir/$uID.$Type.htseq.txt