...
sampleNames <- unique(gsub(paste0("_", Type, "RepeatCounts_half[1-2].rds"), "", basename(countFiles)))
...
for (i in seq(1, length(countFiles), 2)) {
  ...
  if (i==1) {
    Counts <- list(c(readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half1.rds")), readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half2.rds"))))
  } else {
    Counts[[j]] <- c(readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half1.rds")), readRDS(file=paste0(RobjectDir, "/",  expName, "/", Type, "_RepeatCounts/", sampleName, "_cRepeatCounts_half2.rds")))
  }  
  j=j+1
}
