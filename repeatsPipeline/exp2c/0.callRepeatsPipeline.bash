...
for inDir in ${directories[@]}; do
	...
	qsub -q $subType.q -N isc5a_$uID.c -b y -wd $logDir -j y -R y -P \
	GenomeInformatics -pe smp $countcores -V "$RDir --vanilla --args $uID \
	$annot < $scriptsPath/repeatsPipeline/exp2c/5a.countOverlaps_c1.R"
	# 5b.countOverlaps_c2.R:
	qsub -q $subType.q -N isc5b_$uID.c -b y -wd $logDir -j y -R y -P \
	GenomeInformatics -pe smp $countcores -V "$RDir --vanilla --args $uID \
	$annot < $scriptsPath/repeatsPipeline/exp2c/5b.countOverlaps_c2.R"
	
	annot="custom3"
	# 5a.countOverlaps_c1.R:
	qsub -q $subType.q -N iisc5a_$uID.c -b y -wd $logDir -j y -R y -P \
	GenomeInformatics -pe smp $countcores -V "$RDir --vanilla --args $uID \
	$annot < $scriptsPath/repeatsPipeline/exp2c/5a.countOverlaps_c1.R"
	# 5b.countOverlaps_c2.R:
	qsub -q $subType.q -N iisc5b_$uID.c -hold_jid sc3b_$uID -b y -wd $logDir -j y -R y -P \
	GenomeInformatics -pe smp $countcores -V "$RDir --vanilla --args $uID \
	$annot < $scriptsPath/repeatsPipeline/exp2c/5b.countOverlaps_c2.R"
done;
