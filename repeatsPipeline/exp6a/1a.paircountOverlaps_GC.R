...
i=1
chek <- NULL
if (exists("bamPairList")) {
  rm(bamPairList)
}

bamPairList <- sapply(bamGR, function(x) {
  if (!x$qnames %in% chek) {
...
bamGRlist <- GRangesList(bamPairList[!sapply(bamPairList, is.null)])

# save as RDS object:
saveRDS(bamGRlist, file=paste0(RobjectDir, "/", uID, "_bamGRlist.rds"))
