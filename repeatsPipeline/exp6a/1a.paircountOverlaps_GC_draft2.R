...
# to test: qsub -q short.q -N pair2coGC_$uID -b y -wd /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/repeatsPipeline/exp6a/logs -j y -R y -P GenomeInformatics -pe smp 12 -V "/home/jamtor/local/lib/r/R-3.2.2/bin/R --vanilla < /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/repeatsPipeline/exp6a/1a.paircountOverlaps_GC.R"
...
i=1
bamGRlist <- sapply(bamGR, function(x) {
  if (i==1) {
    bamGRlist <- GRangesList(x)
  } else {
    bamGRlist[[i]] <- x
  }
  return(unlist(bamGRlist))
  i <<- i+1
})
...
i=1
bamPairList <- lapply(bamGRlist, function(x) {
  if (!x$qnames %in% chek) {
    pair <- bamGR[grep(x$qnames, bamGR$qnames),]
...
bamPairGR <- GRangesList(bamPairList[!sapply(bamPairList, is.null)])
...
i=1
chek <- NULL
if (exists("testPairList")) {
  rm(testPairList)
}

testPairList <- sapply(testGR, function(x) {
  if (!x$qnames %in% chek) {
...
testPairList <- sapply(testGR, function(x) {
  if (!x$qnames %in% chek) {
    ...
    for (j in 1:length(pair)) {
      if (!pair[j]$flags %in% samflag) {
        if (as.character(strand(pair[j])) == "+") {
          print("Changing 2nd mate to -")
          strand(pair[j]) <- "-"
        } else if (as.character(strand(pair[j])) == "-") {
          strand(pair[j]) <- "+"
          print("Changing 2nd mate to +")
        } else {
...
# remove NULL elements and convert into GR list:
testGRlist <- GRangesList(testPairList[!sapply(testPairList, is.null)])
