...
# load in gc counts, append each half for eah sample and add into list:
countFiles <- grep(
  "REMOVEME", grep(
    paste0("subset"), 
    list.files(inDir, pattern="gc.htseq", 
...
if (!file.exists(paste0(RobjectDir, "/gc_allcounts.htseq.rds"))) {
  saveRDS(gcCounts, file=paste0(RobjectDir, "/gc_allcounts.htseq.rds"))
}
