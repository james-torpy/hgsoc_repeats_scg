...
# remove fastq files as they are no longer needed:
echo -e
echo "Removing fastq files..."
if [ -e $fastqPath/$uID.1.fastq.gz ]; then
    rm $fastqPath/$uID.1.fastq.gz
fi;

if [ -e $fastqPath/$uID.2.fastq.gz ]; then
    rm $fastqPath/$uID.2.fastq.gz
fi;

if [ -e $fastqPath/$uID.unpaired.1.fastq.gz ]; then
    rm $fastqPath/$uID.unpaired.1.fastq.gz
fi;

if [ -e $fastqPath/$uID.unpaired.2.fastq.gz ]; then
    rm $fastqPath/$uID.unpaired.2.fastq.gz
fi;
...
if [ -e $rawDir/fullsamples/bowtell_primary/$uID.bam ]; then
    rm $rawDir/fullsamples/bowtell_primary/$uID.bam
fi;
...
#input/output:
outPath="$projectDir/results/star/GC/$expName/$draft/"
