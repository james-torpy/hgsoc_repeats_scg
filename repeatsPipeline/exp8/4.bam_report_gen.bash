...
# set in/outPaths:
bamPath="$resultsDir/star/GC/$expName/$draft/$uID/"
reportDir="$projectDir/reports/$expName/$draft/"
...
if [ -a $fastqPath/$uID.1.fastq.gz ]; then
	rm $fastqPath/$uID.1.fastq.gz
fi;

if [ -a $fastqPath/$uID.2.fastq.gz ]; then
...
if [ -a $bamPath/Aligned.out.bam ]; then
