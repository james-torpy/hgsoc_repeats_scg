...
os.system('echo ' + str(slots) + ' >> /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test3.txt')
    ...
    for infile in in_files:
        ...
        for i in range(total_no_jobs):
            ...
            if not all([os.path.isfile(inp) for inp in inputs]):
                ...
                os.system('echo /opt/gridengine/bin/linux-x64/qsub ' \
                      + '-q ' + q + '.q ' \
                      + '-N j' + str(i) + '_' + u_id \
                      + holdj \
                      + ' -b y -wd ' \
                      ...
                      + ' '.join(inputs) + ' ' \
                      + ' '.join(outputs) + ' ' \
                      + extra_params[i] + '>> /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test4.txt'
                    )

                os.system('export submit_line="/opt/gridengine/bin/linux-x64/qsub ' \
                      + '-q ' + q + '.q ' \
                      + '-N j' + str(i) + '_' + u_id \
                      + holdj \
                      + ' -b y -wd ' \
                ...
                os.system('/bin/bash ' + script_dir + '/jobSubmit.bash')
