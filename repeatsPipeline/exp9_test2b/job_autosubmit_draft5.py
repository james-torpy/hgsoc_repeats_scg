    ...
    for infile in in_files:
        ...
        for i in range(total_no_jobs):
            ...
            if not all([os.path.isfile(inp) for inp in inputs]):
                ...
                tempfile = record_dir + '/' + 'j' + str(i) + '_' + u_id + '_temp.txt'
                temp = open(tempfile, 'w')
                for v in [q, 'j' + str(i) + '_' + u_id, holdj, log_dir, cores[i], \
                	' '.join(inputs), ' '.join(outputs), extra_params[i]]:
                	temp.write('%s\n' % v)
                ...
                os.system('echo /opt/gridengine/bin/linux-x64/qsub ' \
                      ...
                      + ' '.join(inputs) + ' ' \
                      + ' '.join(outputs) + ' ' \
                      + extra_params[i] + '>> ' + record_dir + '/qsubs.txt'
                    )
