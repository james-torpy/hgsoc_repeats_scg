...
# */10 * * * * /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/record/exp9_test2/qsubs.bash
...
#os.system('echo "pass" > /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test2.txt')
...
total_no_jobs = 4
q = 'long'
python_path = '/home/jamtor/local/bin/Python-3.6.3/bin/python3.6'
script_name = 'job_autosubmit.py'
...
record_dir = project_dir + '/record/' + exp_name
...
# set up new qsub script:
qrec = open(record_dir + '/qsubs.bash', 'w')
...
os.system("qstat -u '*' | grep jamtor | grep -v QRLOGIN | awk '{print $3}' > " + record_dir + "/qstat_jobs.txt")

# fetch job ids currently running or queued:
qstat_jobs = []
with open(record_dir + '/qstat_jobs.txt') as f:
    for line in f:
        qstat_jobs.append(
            re.sub(
                '\n', '', line
...
os.system("qstat -u '*' | grep jamtor | grep -e 'q@' | grep -v QRLOGIN | awk '{print $9}' > " + record_dir + "/qstat_cores.txt")

# fetch core number:
slots = 0
with open(record_dir + '/qstat_cores.txt') as f:
    for line in f:
        slots = slots + int(re.sub('\\n', '', line))

os.system('echo ' + str(slots) + ' >> /share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/repeatsPipeline/exp9/test/test3.txt')
...
running_jobs = []
if not os.stat(record_dir + '/qstat.txt').st_size == 0:
    index = 0
    qstat = pd.read_csv(record_dir + '/qstat.txt', header = None)
    for line in qstat.iterrows():
        if index == 0:
    ...
    for infile in in_files:
        ...
        for i in range(total_no_jobs):
            ...
            if not all([os.path.isfile(inp) for inp in inputs]):
                ...
                os.system('echo At least one job ' + str(i) + ' input file does not exist, holding job >> /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test4.txt')
                ...
                qrec = open(record_dir + '/qsubs.bash', 'a')
                qrec.write('/opt/gridengine/bin/linux-x64/qsub ' \
                      + '-q ' + q + '.q ' \
                      + '-N j' + str(i) + '_' + u_id \
                      + holdj \
                      + ' -b y -wd ' \
                ...
                os.system('chmod 755 ' + record_dir + '/qsubs.bash')
                ...
                qrec2 = open(record_dir + '/qsubs.bash', 'a')
                qrec2.write('/opt/gridengine/bin/linux-x64/qsub ' \
                      + '-q ' + q + '.q ' \
                      + '-N j' + str(i) + '_' + u_id \
                      + holdj \
                      + ' -b y -wd ' \
                ...
                os.system(' echo Job ' + str(i) + ' has previously been completed, no need to run >> /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test4.txt')
            
            elif 'j' + str(i) + '_' + u_id in qstat_jobs:
                os.system('echo Job ' + str(i) + ' is currently running or queued, no need to run again')
                os.system('echo Job ' + str(i) + ' is currently running or queued, no need to run again >> /share/ScratchGeneral/jamtor/projects/hgsoc_repeats/RNA-seq/logs/exp9_test2/test4.txt')
            
qrec = open(record_dir + '/qsubs.bash', 'a')
qrec.write(python_path + ' ' + script_dir + '/' + script_name)
