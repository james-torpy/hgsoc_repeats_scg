...
if ("repeats" %in% resultTypes) {
  ...
  intReps <- rownames(intReps[[1]])
  
  if ( is.na(FCthresh) & is.na(padjthresh) ) {
    sigGenes <- repGenes
    repGenes$threshold <- FALSE
    repGenes$genes <- gsub("\\:.*$", "", rownames(repGenes))
    repGenes$threshold[repGenes$genes %in% intReps] <- TRUE
  } else if ( is.na(FCthresh) ) {
  ...
  lab$genes <- gsub("\\:.*$", "", lab$genes)
  lab <- lab[lab$genes %in% intReps,]
  
  lab2 <- repGenes[repGenes$padj < 0.5 | repGenes$log2FoldChange > 3.5 | repGenes$log2FoldChange < -3.5,]
  lab2 <- lab2[is.finite(lab2$log2FoldChange),]
